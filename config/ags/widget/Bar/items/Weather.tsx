...
export default () => {
	...
	const weatherIcon: string  = wthr.as((w) => {
	if (w && w.current.condition.text in WEATHER_SYMBOL) {
    return WEATHER_SYMBOL[w.current.condition.text];
  }
  return WEATHER_SYMBOL.default;	});

	const weatherTemp: string= wthr.as((w) => {
		if (w) {
