...
const float DIM_CUTOFF = 0.35;  // 保持不变，因为这是亮度的阈值
const float BRIGHT_CUTOFF = 0.65;  // 保持不变，因为这是亮度的阈值
const float BRIGHTNESS_SCALE = 1.00;  // 新增：整体亮度缩放因子
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (source.x > DIM_CUTOFF) {
        dest.x *= 1.1;  // 降低了高亮部分的增益，从1.2降到1.1
    } else {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (c.x > DIM_CUTOFF) {
                glow.yz += c.yz * weight * 0.2;  // 降低色度的贡献，从0.3降到0.2
                if (c.x <= BRIGHT_CUTOFF) {
                    glow.x += c.x * weight * 0.03;  // 降低暗部泛光强度，从0.05降到0.03
                } else {
                    glow.x += c.x * weight * 0.07;  // 降低亮部泛光强度，从0.10降到0.07
                }
            }
        }
        dest.xyz += glow.xyz;
